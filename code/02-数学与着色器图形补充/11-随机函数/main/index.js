    ...
    this.gl.shaderSource(
      ...
      /*glsl*/ ` 
          ...
          void main(){
            ...
            // float c = random(vPosition.xy);
            
            // gl_FragColor = vec4(c,c,c, 1.0);
            // 用点积函数，将二维坐标转换为一维坐标，特定的二维向量vec2(12.9898,78.233)只是我们选择的一个随机数值，这个值可以是任意的，但是要保证它是一个随机的值，增加随机性
            float c = dot(vPosition.xy,vec2(12.9898,78.233));
            // 用sin函数将一维坐标转换为-1到1的之间的值
            c = sin(c);
            // 这个乘法操作将数值转换到一个比较大的范围，这个值可以是任意的，但是要保证它是一个随机的值，增加随机性
            c = c*43758.5453;

            // 用fract函数将数值转换到0到1的之间的值
            c = fract(c);
            gl_FragColor = vec4(c,c,c, 1.0);
